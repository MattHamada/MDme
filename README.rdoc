...
Both of these files have examples included such as
...
== Database setup
The database schema should be setup after running <tt>rake db:migrate</tt>
To load sample data, you can run rake <tt>db:populate</tt>.  Running
...
production that is used for pushing.  Commits should rarely be made to
production; rather you should merge master to production when ready to deploy.
The main exception is when adding sensitive info like api keys, or imbed full
Urls to mdme.us.  When deploying to production use
